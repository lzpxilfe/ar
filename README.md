...
- **가시권/가시선 (Viewshed / LOS)**: 단일/누적/역방향/선형 가시권, 가시선(LOS) + 프로파일, 히구치 거리대, 곡률·굴절 옵션.
- **비용표면/최소비용경로 (Cost Surface / LCP)**: DEM 경사 기반 이동 시간/에너지 모델링 + LCP + 등시간선/등에너지선(옵션).
...
- 비용/네트워크 도구는 기본적으로 “경사 기반 이동 비용”을 사용하며 실제 도로/식생/토지피복은 반영하지 않습니다.
