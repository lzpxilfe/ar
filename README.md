...
즉, **결과가 래스터/벡터 레이어로 존재**하면 대부분 요약에 포함될 수 있습니다. 다만 “각 도구의 의미를 100% 이해해서 해석”하는 수준까지는 아니고, 현재는 **레이어 기반 통계를 모아 문장화**하는 구조입니다. (추후 도구 출력에 표준 메타데이터/요약지표를 붙이면 더 정확한 해석이 가능합니다.)
...
6. 반경(m) 설정 → **AI 요약 생성**
7. 필요 시 `저장…`으로 Markdown/Text 파일로 내보내기
